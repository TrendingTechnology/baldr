...
function action(cmdObj) {
    ...
    core.log('The base path of the song collection is located at:\n    %s\n', chalk.cyan(config.songbook.path));
    if (cmdObj.projectorPath)
        config.songbook.projectorPath = cmdObj.projectorPath;
    if (config.songbook.projectorPath === 'none')
        config.songbook.projectorPath = null;
    if (config.songbook.projectorPath) {
        core.log('The folder where all projector related files are stored is:\n    %s\n', chalk.green(config.songbook.projectorPath));
    }
    // Maybe bug in commander ?
    if (cmdObj.piapath)
        cmdObj.pianoPath = cmdObj.piapath;
    if (cmdObj.pianoPath)
        config.songbook.pianoPath = cmdObj.pianoPath;
    if (config.songbook.pianoPath === 'none')
        config.songbook.pianoPath = null;
    if (config.songbook.pianoPath) {
        core.log('The folder where all piano related files are stored is:\n    %s\n', chalk.green(config.songbook.pianoPath));
    }
    const library = new songbook.IntermediateLibrary(config.songbook.path, config.songbook.projectorPath, config.songbook.pianoPath);
    ...
    if (cmdObj.clean) {
        ...
        if (mode === 'piano' || mode === 'all') {
            const pianoScore = new songbook.PianoScore(library, cmdObj.groupAlphabetically, cmdObj.pageTurnOptimized);
            pianoScore.compile();
        }
        if (config.songbook.projectorPath) {
            const projectorPath = path.join(config.songbook.projectorPath, 'songs.json');
            fs.writeFileSync(projectorPath, JSON.stringify(library, null, '  '));
