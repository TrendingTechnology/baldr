 ...
 * @param options
 */
export function selectSubset (subsetSelector: SubsetSelector, { sort, elements, elementsCount, firstElementNo, shiftSelector }: SelectSubsetConfiguration): any[] {
  const subset = []
 ...
 */
export function sortObjectsByProperty (property: string): Function {
 ...
 * length 80.
 */
export function shortenText (text: string, options: ShortenTextOptions = <ShortenTextOptions> {}): string {
  if (!text) return ''
 ...
 */
export function convertPropertiesCase (data: any, direction: string = 'snake-to-camel'): object {
 ...
 */
export function formatMultiPartAssetFileName (firstFileName: string, no: number): string {
...
interface AssetTypesSpecification {
...
export class AssetTypes {
  private config_: AssetTypesSpecification
  private allowedExtensions_: object
  ...
  private spreadExtensions_ (): object {
    const out = {}
