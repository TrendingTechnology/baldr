...
import type { MediaResolverTypes, MediaCategoriesTypes, StringIndexedObject } from '@bldr/type-definitions'
...
async function queryWikidata (metaData: MediaResolverTypes.YamlFormat, categoryNames: MediaCategoriesTypes.Names, categoryCollection: MediaCategoriesTypes.Collection): Promise<MediaResolverTypes.YamlFormat> {
  const dataWiki = await wikidata.query(metaData.wikidata, categoryNames, categoryCollection)
  console.log(dataWiki)
  metaData = wikidata.mergeData(metaData, dataWiki, categoryCollection) as MediaResolverTypes.YamlFormat
  // To avoid blocking
