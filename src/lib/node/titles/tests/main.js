...
function makeDeepTitel (filePath) {
  return new DeepTitle(path.join(config.mediaServer.basePath, 'musik', filePath))
}
...
describe('Package “@bldr/titles”', function () {
  describe('Class “DeepTitle”', function () {
    it('07_Hoer-Labyrinth/TX/Arbeitsblatt.tex', function () {
      ...
      assert.strictEqual(deep.curriculum, '5. Jahrgangsstufe / Lernbereich 4: Musik und ihre Grundlagen / Instrumente verschiedener Gruppen')
      assert.strictEqual(deep.curriculumFromGrade, 'Lernbereich 4: Musik und ihre Grundlagen / Instrumente verschiedener Gruppen')
      ...
      assert.strictEqual(deep.titles[3].relPath, 'musik/05/40_Grundlagen/97_Instrumente')
    })
    ...
    it('11/30_Politik/10_Lieder/10_Arbeiterlied-Weber/Praesentation.baldr.yml', function () {
      ...
      assert.strictEqual(deep.allTitles, 'Fach Musik / 11. Jahrgangsstufe / Musik im Dienst politischer Ideen / Politische Lieder / Ein Arbeiterlied des 19. Jahrhunderts')
  ...
  describe('Class “TreeFactory”', function () {
    it('build', function () {
      ...
      function add (relPath) {
        factory.addTitleByPath(path.join(config.mediaServer.basePath, 'musik/06/40_Grundlagen/05_Intervalle-Feinbestimmung', relPath))
      }
