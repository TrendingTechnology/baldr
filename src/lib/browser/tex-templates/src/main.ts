 ...
 */
function foldText (text: string, charCount: number = 72, buildArray: string[] = []): string[] {
...
export function environment (name: string, content: string, pairs?: KeyValuePairs): string {
  let pairsRendered = ''
  if (pairs != null) {
    pairsRendered = '[\n' + keyValues(pairs) + '\n]'
  }
