...
const { wikipediaMaster } = require('../../dist/node/main.js')

describe('Master slide “wikipedia”', function () {
  it('Function “getFirstImage()”', async function () {
    const result = await wikipediaMaster.queryFirstImage(
  ...
  describe('Function “queryHtmlBody()”', function () {
    it('without oldid', async function () {
      this.timeout(10000)
      const result = await wikipediaMaster.queryHtmlBody(
        'Wolfgang_Amadeus_Mozart',
        'de'
      )
    ...
    it('image', async function () {
      this.timeout(10000)
      const result = await wikipediaMaster.queryHtmlBody('Conrad_Martin', 'de')
  ...
  describe('Function “getHtmlBody()”', function () {
    it('throws', async function () {
      this.timeout(10000)
      assert.throws(() => {
        wikipediaMaster.getHtmlBody('Mittelspannungsrichtlinie', 'de')
      })
      let result = await wikipediaMaster.queryHtmlBody(
        'Mittelspannungsrichtlinie',
        'de'
      )
      assert.ok(result.includes('Mittelspannungsrichtlinie'))
      result = wikipediaMaster.getHtmlBody(
        'Mittelspannungsrichtlinie',
        'de'
      )
      assert.ok(result.includes('Mittelspannungsrichtlinie'))
