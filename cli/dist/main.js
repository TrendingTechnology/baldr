    ...
    for (const commandName of subcommandDirs) {
        ...
        if (definition.alias != null) {
            if (!aliases.includes(definition.alias)) {
                ...
                throw new Error(`Duplicate alias “${definition.alias}” used for the (sub)command “${definition.command}”.`);
            }
        }
 ...
 */
export function collectAllOpts(program) {
    const result = {};
...
export function validateDefintion(spec) {
    return spec;
}
async function main() {
