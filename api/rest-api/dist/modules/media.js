...
export default function () {
    ...
    app.get('/presentations/by-:scheme/:authority', async (request, response, next) => {
        try {
            if (request.params.scheme !== 'ref' &&
                request.params.scheme !== 'uuid') {
                throw new Error('Scheme has to be “ref” or “uuid”');
            }
            response.json(await database.getPresentation(request.params.scheme, request.params.authority));
        }
        catch (error) {
            next(error);
        }
    });
    app.get('/get/presentations/by-substring', async (request, response, next) => {
        try {
    ...
    app.get('/assets/by-:scheme/:authority', async (request, response, next) => {
        try {
            ...
            response.json(await database.getAsset(request.params.scheme, request.params.authority));
        }
        catch (error) {
            next(error);
        }
