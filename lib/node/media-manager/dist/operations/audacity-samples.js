...
import { asciify, convertSecondsToHHMMSS, convertHHMMSSToSeconds } from '@bldr/string-format';
 ...
 *   file.
 */
function convertAudacityToYaml(filePath, content) {
...
function convertYamlToAudacity(filePath, content) {
    const yaml = convertFromYaml(content);
    if (yaml.samples == null) {
        throw new Error('No field “samples” found.');
    }
    const lines = [];
    for (const sample of yaml.samples) {
        const startTime = convertHHMMSSToSeconds(sample.startTime);
        const endTime = convertHHMMSSToSeconds(sample.endTime == null ? sample.startTime : sample.endTime);
        const description = sample.title != null ? sample.title + ' ' + sample.ref : sample.ref;
