...
async function queryWikidata(metaData, categoryNames, categoryCollection) {
 ...
 * @param filePath - The media asset file path.
 */
export async function normalizeMediaAsset(filePath, options) {
    try {
 ...
 */
export async function initializeMetaYaml(filePath, metaData) {
 ...
 */
export async function convertAsset(filePath, cmdObj = {}) {
    ...
    if (process != null) {
        ...
        if (process.status === 0) {
            if (mimeType === 'audio') {
                ...
                if (metaData != null) {
                    await writeYamlMetaData(outputFile, metaData);
                }
            }
