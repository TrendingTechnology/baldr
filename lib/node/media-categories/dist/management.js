 ...
 *   `*.extension.yml` file.
 */
async function sortAndDeriveProps(data, category, filePath) {
 ...
 *   `*.extension.yml` file.
 */
async function processByType(data, name, filePath) {
 ...
 *   `*.extension.yml` file.
 */
export async function process(data, filePath) {
    if (filePath != null) {
        filePath = path.resolve(filePath);
    }
    ...
    for (const name of data.categories.split(',')) {
        data = await processByType(data, name, filePath);
    }
