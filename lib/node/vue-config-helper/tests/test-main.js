...
import { buildStyleResourcesLoaderConfig } from '../dist/style-resources-loader'
import { searchForAliases } from '../dist/webpack-aliases'
import { configureVue } from '../dist/main'
...
describe('Package “@bldr/vue-config-helper”', function () {
  it('Function “searchAliases()”', function () {
    const aliases = searchForAliases(
      path.join(config.localRepo, 'vue', 'apps', 'presentation')
    )
  ...
  describe('Function “configureVue()”', function () {
    const dirname = path.join(config.localRepo, 'vue/apps/presentation')
