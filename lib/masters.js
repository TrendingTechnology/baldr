 ...
 * functionality for the master slide classes.
 */
var MasterOfMasters = class {

  constructor(document, data) {
    this.pathMasterOfMasters = __dirname;
  ...
  hasCSS() {
    let parentPath;
    if (this.pathMaster) {
      parentPath = this.pathMaster;
    }
    else if (this.pathMasterOfMasters) {
      parentPath = this.pathMasterOfMasters;
    } else {
      return false;
    }

    if (fs.existsSync(path.join(parentPath, 'styles.css'))) {
      return true;
    }
    else {
      return false;
  ...
  setCSS() {
    if (this.hasCSS()) {
      ...
      link.rel = 'stylesheet';
      link.type = 'text/css';
      link.href = path.join(this.pathMaster, 'styles.css');
