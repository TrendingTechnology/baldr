...
export function convertHHMMSSToSeconds (duration: string | number): number {
  ...
  if (typeof duration === 'number') {
    ...
    if (segments.length < 2 || segments.length > 3) {
      throw new Error(
        `Invalid duration string ${duration}: Only one or two colons are allowed!`
      )
    }
    ...
    if (seconds >= 60) {
      throw new Error(
        `Invalid duration string “${duration}”: The number of seconds must be less than 60!`
      )
    }
  } else {
  ...
  if (minutes >= 60) {
    throw new Error(
      `Invalid duration string “${duration}”: The number of minutes must be less than 60!`
    )
  }
