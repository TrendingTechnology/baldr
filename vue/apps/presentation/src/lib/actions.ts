...
function toggleLastRoute (routeNameTo: string): void {
  if (router.currentRoute.name === routeNameTo) {
    const lastRoute = store.getters['presentation/nav/lastRoute']
    if (lastRoute != null) {
      store.commit('presentation/nav/lastRoute', null)
      router.push(lastRoute).catch(logCatch)
    }
  } else {
    store.commit('presentation/nav/lastRoute', router.currentRoute)
...
function callOpenRestApi (
  ...
  const presentation = store.getters['presentation/presentationNg']
...
function goToNextSlideByDirection (direction: 1 | -1): void {
  ...
  const slides = store.getters['presentation/slides']
  const slidesCount = store.getters['presentation/slidesCount']
  ...
  // next
  if (direction === 1) {
    store.dispatch('presentation/highlightCursorArrow', 'right').catch(logCatch)
    // previous
  } else if (direction === -1) {
    store.dispatch('presentation/highlightCursorArrow', 'left').catch(logCatch)
  }
...
function goToNextStepByDirection (direction: 1 | -1): void {
  ...
  // next
  if (direction === 1) {
    store.dispatch('presentation/highlightCursorArrow', 'down').catch(logCatch)
    // previous
  } else if (direction === -1) {
    store.dispatch('presentation/highlightCursorArrow', 'up').catch(logCatch)
  }
...
function goToNextSlideOrStepByDirection (direction: 1 | -1): void {
  ...
  const params = store.getters['presentation/nav/nextRouterParams'](direction)
  ...
  if (direction === 1) {
    ...
    if (
      params.stepNo != null &&
      params.stepNo !== store.getters['presentation/slide'].stepCount
    ) {
      store.dispatch('presentation/highlightCursorArrow', 'up').catch(logCatch)
    } else {
      store.dispatch('presentation/highlightCursorArrow', 'left').catch(logCatch)
...
export function resetSlideScaleFactor (): void {
  store.dispatch('presentation/resetSlideScaleFactor').catch(logCatch)
}

export function increaseSlideScaleFactor (): void {
  store.dispatch('presentation/increaseSlideScaleFactor').catch(logCatch)
}

export function decreaseSlideScaleFactor (): void {
  store.dispatch('presentation/decreaseSlideScaleFactor').catch(logCatch)
}

export function toggleSlides (): void {
  store.dispatch('presentation/setSlideNoToOld').catch(logCatch)
}

export async function update (): Promise<void> {
  try {
    const result = await api.updateMediaServer()
    store.dispatch('presentation/titles/loadRootTreeList').catch(logCatch)
...
export function toggleMetaDataOverlay (): void {
  store.dispatch('presentation/toggleMetaDataOverlay').catch(logCatch)
}
