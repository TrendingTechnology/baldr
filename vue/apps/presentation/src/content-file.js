 ...
 *
 * @module @bldr/presentation/content-file
 */
...
async function addAssetsNgToStore (uris, throwException = true) {
  ...
  for (const asset of assets) {
    store.commit('presentation/media/addAsset', asset)
  }
  const samples = resolver.exportSamples()
  for (const sample of samples) {
    store.commit('presentation/media/addSample', sample)
  }
  return assets
}
...
export class Slide {
  /**
   * @param {module:@bldr/presentation/content-file~rawSlideData} rawSlideData - The
  ...
  constructor (rawSlideData) {
    ...
    /**
     * @type {module:@bldr/presentation/masters~Master}
     */
     ...
     * The slide steps.
     *
     * @type {module:@bldr/presentation/steps~SlideStep[]}
     */
    this.steps = null

    /**
     * @type {module:@bldr/presentation/content-file.MetaData}
     */
    ...
    /**
     * @type {module:@bldr/presentation/content-file~AudioOverlay}
     */
  ...
  get assets () {
    ...
    for (const mediaUri of this.mediaUris) {
      assets.push(store.getters['presentation/media/assetByUri'](mediaUri))
    }
    return assets
  }
...
export class Presentation {
  ...
  async resolveMedia (vmExternal) {
    ...
    store.dispatch('presentation/nav/initNavList', this.slides)
  }
   ...
   */
  goto (slideId) {
    const slideNo = this.store.getters['presentation/nav/slideNoById'](slideId)
