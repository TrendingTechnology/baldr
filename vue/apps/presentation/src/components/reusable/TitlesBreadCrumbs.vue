...
const mapTitlesGetters = createNamespacedHelpers('presentation/titles').mapGetters
...
@Component({
  computed: {
    ...mapPresentationGetters(['presentation']),
...
export default class TitlesBreadCrumbs extends Vue {
  ...
  mounted (): void {
    if (this.rootTreeList == null) {
      this.$store.dispatch('presentation/titles/loadRootTreeList')
    }
  }
}
</script>
